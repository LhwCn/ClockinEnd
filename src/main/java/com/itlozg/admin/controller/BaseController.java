...
import com.itlozg.admin.entity.vo.DataGrid;
import com.itlozg.admin.model.request.PageRequest;
...
//    }
    ...
    private class StringEscapeEditor extends PropertyEditorSupport {
...
//                }
                if (escapeJavaScript || text.toLowerCase().indexOf("alert(") > -1 || text.toLowerCase().indexOf("<script") > -1) {
                    if (isJsonStr(value)) {
        ...
        private boolean isJsonStr(String strVal) {
            boolean result = false;
            try {
                JSONObject.parse(strVal);
                result = true;
            } catch (Exception e) {
                result = false;
            }
